...
def match_topics(syllabus_json: dict, lecture_json: dict, course_name: str) -> List[Dict]:
    ...
    save_json(result, "/workspaces/swayam/result/BS/2907_FA_LU_matched_topics_local.json")
    return result
...
def match_subtopics(syllabus_json: dict, lecture_json: dict, course_name: str, with_context: bool = True) -> List[Dict]:
    ...
    save_json(hierarchical_result, "/workspaces/swayam/result/BS/2907_FA_LU_matched_subtopics_local.json")
...
def generate_summary(match_results: List[Dict], key="similarity", label="Topic"):
    ...
    df[df["similarity"] < 95].to_csv(f"FA_LU_no_match_{label.lower()}_95.csv", index=False)
    df[df["similarity"] < 85].to_csv(f"FA_LU_no_match_{label.lower()}_85.csv", index=False)
    df[df["similarity"] < 75].to_csv(f"FA_LU_no_match_{label.lower()}_75.csv", index=False)
    df[df["similarity"] < 50].to_csv(f"FA_LU_no_match_{label.lower()}_50.csv", index=False)
...
if __name__ == '__main__':
    ...
    lecture_data = load_json("/workspaces/swayam/result/2106V2All_hierarchical_lecture_topics.json")
