...
def match_topics(syllabus_json: dict, lecture_json: dict, course_name: str) -> List[Dict]:
    ...
    save_json(result, "/workspaces/swayam/result/BS/2907_FA_LU_IGNOU_matched_topics_local.json")
    return result
...
def match_subtopics(syllabus_json: dict, lecture_json: dict, course_name: str, with_context: bool = True) -> List[Dict]:
    ...
    save_json(hierarchical_result, "/workspaces/swayam/result/BS/2907_FA_LU_IGNOU_matched_subtopics_local.json")
...
def generate_summary(match_results: List[Dict], key="similarity", label="Topic"):
    ...
    df[df["similarity"] < 95].to_csv(f"FA_LU_IGNOU_no_match_{label.lower()}_95.csv", index=False)
    df[df["similarity"] < 85].to_csv(f"FA_LU_IGNOU_no_match_{label.lower()}_85.csv", index=False)
    df[df["similarity"] < 75].to_csv(f"FA_LU_IGNOU_no_match_{label.lower()}_75.csv", index=False)
    df[df["similarity"] < 50].to_csv(f"FA_LU_IGNOU_no_match_{label.lower()}_50.csv", index=False)

# ------------------- SAMPLE USAGE -------------------
if __name__ == '__main__':
    start = time.time()
    course = "Financial Accounting"
    syllabus_data = load_json("/workspaces/swayam/result/LU_syllabus_extracted_topics_subtopics_new.json")  #/workspaces/swayam/result/DU_syllabus_extracted_topics_subtopics.json /workspaces/swayam/result/ugc_corrected_syllabus_new.json /workspaces/swayam/result/LU_syllabus_extracted_topics_subtopics_new.json  /workspaces/swayam/result/BS/raw/BHU_FINANCIAL_ACCOUNTING_syllabus0107.json
    lecture_data = load_json("/workspaces/swayam/result/0108FAIGNOU_hierarchical_lecture_topics.json") # IIT Bombay FA - /workspaces/swayam/result/2106V2All_hierarchical_lecture_topics.json
