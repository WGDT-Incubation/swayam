...
def match_topics(syllabus_json: dict, lecture_json: dict, course_name: str) -> List[Dict]:
    ...
    save_json(result, "/workspaces/swayam/result/BS/2907_mani_BS_matched_topics_local.json")
    return result
...
def match_subtopics(syllabus_json: dict, lecture_json: dict, course_name: str, with_context: bool = True) -> List[Dict]:
    ...
    save_json(hierarchical_result, "/workspaces/swayam/result/BS/2907_mani_BS_matched_subtopics_local.json")
...
def generate_summary(match_results: List[Dict], key="similarity", label="Topic"):
    ...
    df[df["similarity"] < 95].to_csv(f"du_mani__no_match_{label.lower()}_95.csv", index=False)
    df[df["similarity"] < 85].to_csv(f"du_mani__no_match_{label.lower()}_85.csv", index=False)
    df[df["similarity"] < 75].to_csv(f"du_mani__no_match_{label.lower()}_75.csv", index=False)
    df[df["similarity"] < 50].to_csv(f"du_mani_no_match_{label.lower()}_50.csv", index=False)

# ------------------- SAMPLE USAGE -------------------
if __name__ == '__main__':
    start = time.time()
    course = "BUSINESS_STATISTICS"
    syllabus_data = load_json("/workspaces/swayam/result/BS/raw/Manipal_BUSINESS_STATISTICS_syllabus0107.json")  #bharatividyapeeth
    lecture_data = load_json("/workspaces/swayam/result/BS/2506_BS_hierarchical_lecture_topics.json")
